 ...
 * ```
 */
class CWLogsWritable extends stream_1.Writable {
    constructor(options) {
    ...
    set logStreamName(logStreamName) {
    ...
    validateOptions(options) {
        ...
        if (hasOwnProperty.call(options, "retryableDelay") &&
        ...
        if (hasOwnProperty.call(options, "maxBatchCount") &&
        ...
        if (hasOwnProperty.call(options, "maxBatchSize") &&
        ...
        if (hasOwnProperty.call(options, "onError") &&
            typeof options.onError !== "function") {
            throw new Error("onError option must be a function, if specified");
        }
        if (hasOwnProperty.call(options, "filterWrite") &&
     ...
     * @returns {string}
     */
    safeStringifyLogEvent(rec) {
        return (0, fast_safe_stringify_1.default)(rec);
    }
    /**
     ...
     */
    createService(opts) {
        return new client_cloudwatch_logs_1.CloudWatchLogs(opts);
    }
    /**
    ...
    dequeueNextLogBatch() {
        ...
        for (var i = 0, l = this.queuedLogs.length; i < l; i++) {
            ...
            if (dropLogEvent) {
                ...
                if (sizeEstimate > this.maxBatchSize ||
    ...
    _sendLogs() {
        ...
        if (this.sequenceToken === null) {
            this._getSequenceToken(logGroupName, logStreamName, (err, sequenceToken) => {
                if (err) {
        ...
        this._putLogEvents(apiParams, (err, sequenceToken) => {
            if (err) {
     ...
     * @private
     */
    _nextAfterError(_onErrorNextCbId, errOrLogEvents) {
        ...
        function attemptPut() {
            cloudwatch.putLogEvents(apiParams, function (err, res) {
                if (err) {
                    ...
                    cb(null, res === null || res === void 0 ? void 0 : res.nextSequenceToken);
                }
            });
        }
    }
     ...
     * @private
     */
    _getSequenceToken(logGroupName, logStreamName, cb) {
        this.cloudwatch.describeLogStreams({
            ...
            if (err) {
                if (err.name === "ResourceNotFoundException") {
                    this._createLogGroupAndStream(logGroupName, logStreamName, (err) => {
                        if (err) {
                            cb(err);
                        }
                        else {
            ...
            else if (((_a = data === null || data === void 0 ? void 0 : data.logStreams) === null || _a === void 0 ? void 0 : _a.length) === 0) {
                ...
                cb(null, (_b = data === null || data === void 0 ? void 0 : data.logStreams) === null || _b === void 0 ? void 0 : _b[0].uploadSequenceToken);
            }
        });
    }
    /**
     ...
     * @private
     */
    _createLogGroupAndStream(logGroupName, logStreamName, cb) {
     ...
     * @private
     */
    _createLogGroup(logGroupName, cb) {
     ...
     * @private
     */
    _createLogStream(logGroupName, logStreamName, cb) {
     ...
     * @param {string} logEventMessage - Stringified log event.
     */
    _emitOversizeLogEvent(logEventMessage) {
        this.emit("oversizeLogEvent", logEventMessage);
    }
}
exports.CWLogsWritable = CWLogsWritable;
