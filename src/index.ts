...
const MAX_MESSAGE_SIZE = 262118;

type Next = (error?: Error | null, ...extra: any[]) => void;
type RawLogInput = string | Record<string, any>;
...
export interface CWLogsWritableOptions {
  logGroupName: string; // AWS CloudWatch [LogGroup](http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/CloudWatchLogs.html#putLogEvents-property) name. It will be created if it doesn't exist.
  logStreamName: string; // AWS CloudWatch [LogStream](http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/CloudWatchLogs.html#putLogEvents-property) name. It will be created if it doesn't exist.
  cloudWatchLogsOptions: object; //={}] - Options passed to [AWS.CloudWatchLogs](http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/CloudWatchLogs.html#constructor-property) service.
  writeInterval: number | "nextTick"; // =nextTick] - Amount of wait time after a Writable#_write call to allow batching of log events. Must be a positive number or "nextTick". If "nextTick", `process.nextTick` is used. If a number, `setTimeout` is used.
  retryableDelay: number | "nextTick"; // =150]
  retryableMax: number; // Default =100] - Maximum number of times an AWS error marked as "retryable" will be retried before the error is instead passed to {@link CWLogsWritable#onError}.
  maxBatchCount: number; // Default =10000] - Maximum number of log events allowed in a single PutLogEvents API call.
  maxBatchSize: number; // Default =1048576] - Maximum number of bytes allowed in a single PutLogEvents API call.
  ignoreDataAlreadyAcceptedException: boolean; // Default =true] - Ignore `DataAlreadyAcceptedException` errors. This will bypass {@link CWLogsWritable#onError}. See [cwlogs-writable/issues/10](https://github.com/amekkawi/cwlogs-writable/issues/10).
  retryOnInvalidSequenceToken: boolean; // Default =true] - Retry on `InvalidSequenceTokenException` errors. This will bypass {@link CWLogsWritable#onError}. See [cwlogs-writable/issues/12](https://github.com/amekkawi/cwlogs-writable/issues/12).
  onError: (err: Error, logEvents: InputLogEvent[] | null, next: Next) => void; // Default ] - Called when an AWS error is encountered. Overwrites {@link CWLogsWritable#onError} method.
  filterWrite: (rec: RawLogInput) => boolean; // Default ] - Filter writes to CWLogsWritable. Overwrites {@link CWLogsWritable#filterWrite} method.
  objectMode: boolean; // Default =true] - Passed to the Writable constructor. See https://nodejs.org/api/stream.html#stream_object_mode.
}
 ...
 * ```
 */
export class CWLogsWritable extends Writable {
  private _onErrorNextCbId = 1;
  public sequenceToken = null;
  public writeQueued = false;
  public readonly cloudwatch;
   ...
   */
  public queuedLogs: InputLogEvent[] = [];

  public writeInterval: number | "nextTick";
  public ignoreDataAlreadyAcceptedException: boolean;
  public retryOnInvalidSequenceToken: boolean;
  public retryableMax: number;
  public retryableDelay: number | "nextTick";
  ...
  constructor(options: CWLogsWritableOptions) {
    ...
    this.cloudwatch = this.createService(options.cloudWatchLogsOptions || {});
     ...
     */
    this.writeInterval =
      typeof options.writeInterval === "number"
     ...
     * @default true
     */
    this.ignoreDataAlreadyAcceptedException =
      options.ignoreDataAlreadyAcceptedException !== false;
     ...
     * @default true
     */
    this.retryOnInvalidSequenceToken =
      options.retryOnInvalidSequenceToken !== false;
     ...
     */
    this.retryableMax =
      typeof options.retryableMax === "number"
     ...
     */
    this.retryableDelay =
      options.retryableDelay === "nextTick" ||
      typeof options.retryableDelay === "number"
     ...
     */
    this.maxBatchCount =
      typeof options.maxBatchCount === "number"
  ...
  set logGroupName(logGroupName: string) {
    if (logGroupName !== this._logGroupName) {
  ...
  set logStreamName(logStreamName: string) {
    if (logStreamName !== this._logStreamName) {
   ...
   * @throws Error
   */
  validateOptions(options: CWLogsWritableOptions) {
    ...
    if (
      hasOwnProperty.call(options, "objectMode") &&
      typeof options.objectMode !== "boolean"
    ) {
      throw new Error("objectMode option must be a boolean, if specified");
    }

    if (
      hasOwnProperty.call(options, "writeInterval") &&
      !isInterval(options.writeInterval)
    ) {
      throw new Error(
        'writeInterval option must be a positive number or "nextTick", if specified'
      );
    }

    if (
      hasOwnProperty.call(options, "ignoreDataAlreadyAcceptedException") &&
      typeof options.ignoreDataAlreadyAcceptedException !== "boolean"
    ) {
      throw new Error(
        "ignoreDataAlreadyAcceptedException option must be a boolean"
      );
    }

    if (
      hasOwnProperty.call(options, "retryOnInvalidSequenceToken") &&
      typeof options.retryOnInvalidSequenceToken !== "boolean"
    ) {
      throw new Error("retryOnInvalidSequenceToken option must be a boolean");
    }

    if (
      hasOwnProperty.call(options, "retryableMax") &&
      (!isFiniteNumber(options.retryableMax) || options.retryableMax < 1)
    ) {
      throw new Error(
        "retryableMax option must be a non-zero positive number, if specified"
      );
    }

    if (
      hasOwnProperty.call(options, "retryableDelay") &&
      !isInterval(options.retryableDelay)
    ) {
      throw new Error(
        'retryableDelay option must be a positive number or "nextTick", if specified'
      );
    }

    if (
      hasOwnProperty.call(options, "maxBatchCount") &&
      (!isFiniteNumber(options.maxBatchCount) ||
        options.maxBatchCount < 1 ||
        options.maxBatchCount > 10000)
    ) {
      throw new Error(
        "maxBatchCount option must be a positive number from 1 to 10000, if specified"
      );
    }

    if (
      hasOwnProperty.call(options, "maxBatchSize") &&
      (!isFiniteNumber(options.maxBatchSize) ||
        options.maxBatchSize < 256 ||
        options.maxBatchSize > 1048576)
    ) {
      throw new Error(
        "maxBatchSize option must be a positive number from 256 to 1048576, if specified"
      );
    }

    if (
      hasOwnProperty.call(options, "onError") &&
    ...
    if (
      hasOwnProperty.call(options, "filterWrite") &&
      typeof options.filterWrite !== "function"
    ) {
  ...
  createLogEvent(rec: string | Record<string, any>): InputLogEvent {
    return {
      message: typeof rec === "string" ? rec : this.safeStringifyLogEvent(rec),

      timestamp:
        typeof rec === "object" && rec.time
   ...
   * @returns {string}
   */
  safeStringifyLogEvent(rec: Record<string, any>) {
    return safeStringify(rec);
  }
   ...
   * ```
   */
  onError(err: Error, logEvents: InputLogEvent[] | null, next: Next) {
    next(err);
  }
   ...
   */
  private createService(opts: CloudWatchLogsClientConfig) {
    return new CloudWatchLogs(opts);
  }
  ...
  dequeueNextLogBatch() {
    ...
    for (var i = 0, l = this.queuedLogs.length; i < l; i++) {
      ...
      if (dropLogEvent) {
        ...
        if (
          sizeEstimate > this.maxBatchSize ||
          batchCount - dropIndexes.length >= this.maxBatchCount
        ) {
          break;
        }
      }
   ...
   */
  reduceOversizedMessage(logEventMessage: string) {
   ...
   * @private
   */
  _write(record: RawLogInput, _enc: string, cb: Next) {
    ...
    if (this.filterWrite(record)) {
      ...
      try {
        ...
        err.message =
          "Error while stringifying record (install safe-json-stringify to fallback to safer stringification) -- " +
          err.message;
  ...
  _sendLogs() {
    ...
    this._putLogEvents(apiParams, (err: any, sequenceToken) => {
      if (err) {
   ...
   * @private
   */
  _nextAfterError(_onErrorNextCbId: number, errOrLogEvents?: any) {
   ...
   * @private
   */
  _putLogEvents(apiParams: PutLogEventsCommandInput, cb: Next) {
    const retryableDelay = this.retryableDelay;
    const retryableMax = this.retryableMax;
    const cloudwatch = this.cloudwatch;
    ...
    function attemptPut() {
      cloudwatch.putLogEvents(
        apiParams,
        function (err: any, res?: PutLogEventsCommandOutput) {
          if (err) {
   ...
   * @private
   */
  _getSequenceToken(logGroupName: string, logStreamName: string, cb: Next) {
    this.cloudwatch.describeLogStreams(
      {
        logGroupName: logGroupName,
        logStreamNamePrefix: logStreamName,
      },
      (err: any, data?: DescribeLogStreamsCommandOutput) => {
        if (err) {
          if (err.name === "ResourceNotFoundException") {
            this._createLogGroupAndStream(
              logGroupName,
              logStreamName,
              (err) => {
                if (err) {
          ...
          this._createLogStream(logGroupName, logStreamName, (err: any) => {
            if (err) {
              cb(err);
            } else {
   ...
   * @private
   */
  _createLogGroupAndStream(
    logGroupName: string,
    logStreamName: string,
    cb: Next
  ) {
    this._createLogGroup(logGroupName, (err) => {
      if (err) {
        cb(err);
      } else {
        this._emitCreateLogGroup();
        this._createLogStream(logGroupName, logStreamName, (err) => {
          if (err) {
            cb(err);
          } else {
            this._emitCreateLogStream();
   ...
   * @private
   */
  _createLogGroup(logGroupName: string, cb: Next) {
   ...
   * @private
   */
  _createLogStream(logGroupName: string, logStreamName: string, cb: Next) {
   ...
   */
  _emitPutLogEvents(logEvents: InputLogEvent[]) {
   ...
   * @param {object|string} rec
   */
  _emitStringifyError(err: Error, rec: RawLogInput) {
   ...
   * @param {string} logEventMessage - Stringified log event.
   */
  _emitOversizeLogEvent(logEventMessage: string) {
...
function isInterval(val: number | string) {
  return val === "nextTick" || (isFiniteNumber(val) && val >= 0);
}

function logEventComparator(a: InputLogEvent, b: InputLogEvent) {
